...
int main(int argc, char *argv[]) {
#ifndef NDEBUG
	printf("**** kDC-two (Debug) build at %s %s ***\n", __TIME__, __DATE__);
	printf("!!! You may want to define NDEBUG in Utility.h to get better performance!\n");
#else
	printf("**** kDC-two (Release) build at %s %s ***\n", __TIME__, __DATE__);
#endif
#ifdef _RR_OPT_
	printf("*** RR_OPT is enabled ***\n");
#endif
	...
	op.add<Switch>("t", "two_stage", "\'use the two stage paradigm\'", &two_stage);
	...
	else if(strcmp(alg.c_str(), "exact") == 0) graph->kDefectiveClique_exact(two_stage);
