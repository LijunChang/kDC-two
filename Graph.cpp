...
void Graph::kDefectiveClique_exact(bool two_stage) {
	Timer t;
	...
	if(kDefectiveClique.size() < UB) {
		...
		if(two_stage) {
			...
			for(ui i = n;i > 0&&kDefectiveClique.size() < UB;i --) {
				...
				if(kDefectiveClique.size() > K+1) extract_subgraph_with_prune(u, peel_sequence_rid, degree, ids, rid, vp, vis, pstart, pend, edges);
				else extract_subgraph(u, peel_sequence_rid, ids, rid, vp, vis, pstart, pend, edges);
		...
		if(two_stage&&kDefectiveClique.size() < K+1) UB = K+1;
